...
<li class="nav-item">
    ...
    class="nav-link
   @if(Request::fullUrl() === (!empty($item->type) ? route($item->route, ['type' => $item->type]) : route($item->route)))
   activeSidebar
   @endif" data-key="t-detached">{{ ucwords($item->name) }}</a>
    @else
    <a class="nav-link" href="#{{ $item->id }}"
         data-bs-toggle="collapse" role="button" aria-expanded="true" aria-controls="{{ $item->id }}">
         @if($item->icon)
         <i class="{{ $item->icon }}"></i>
         @endif

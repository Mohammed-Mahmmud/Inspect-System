        ...
        <div class="page-content">
            <div class="container-fluid">
                ...
                <div class="row">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table align-middle mb-0">
                                            ...
                                            <tr>
                                                ...
                                                <th class="sort"
                                                    data-sort="customer_number">{{ trans('Dashboard/orders.number') }}</th>
                                                <th class="sort"
                                                    data-sort="customer_company">{{ trans('Dashboard/orders.company') }}</th>
                                                <th class="sort"
                                                    data-sort="customer_rig_id">{{ trans('Dashboard/orders.rig') }}</th>
                                                <th class="sort"
                                                    data-sort="action">{{ trans('Dashboard/orders.create_date') }}</th>
                                                <th class="sort"
                                                    data-sort="action">{{ trans('Dashboard/orders.action') }}</th>
                                            </tr>
                                            </thead>
                                                                        ...
                                                                        <div class="modal-content">
                                                                            <div class="modal-header" S>
                                                                                <h5 class="modal-title"
                                                                                    ...
                                                                                    style="word-wrap: break-word; overflow-wrap: break-word;">{{ trans('Dashboard/orders.remove') }} {{ $item->name}}</h5>
                    ...
                    {{-- update form --}}
                    <form class="tablelist-form" action="{{ route('order.update',$item) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    <div class="modal-header bg-light p-3">
                                        <h5 class="modal-title"
                                            id="exampleModalLabel2">{{ trans('Dashboard/orders.edit').' '.$item->name }}</h5>
                                        ...
                                        <div class="modal-body">
                                            <div class="mb-3">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <x-form.input_label class="col-12" placeholder="Enter Report Date"
                                                                        value="{{ $item->date ?? '' }}"
                                                                        label="Report Date :" type="date" name="date"
                                                                        id="date" required/>
                                                ...
                                                <div class="row">
                                                    <div class="col-6">
                                                        ...
                                                        <select
                                                            ...
                                                            <option value="{{ optional($item->companies)->id }}"
                                                                    selected
                                                                    disabled>{{ $item->companies->name }}</option>
                                                            @foreach ($companies as $company)
                                                                <option
                                                                    value="{{ $company->id }}" {{ $company->id == $item->companies->id ? 'selected' : '' }}>{{ $company->name }}</option>
                                                            @endforeach
                                                        </select>
                                                        <span
                                                    ...
                                                    <div class="col-6">
                                                        ...
                                                        <select
                                                            ...
                                                            id="updated_rig_id" name="rig_id">
                                                            <option value="" selected
                                                                    disabled>{{optional($item->rigs)->name }}</option>
                                                            @foreach ($rigs as $rig)
                                                                <option
                                                                    value="{{ $rig->id }}" {{ $rig->id == optional($item->rigs)->id ? 'selected' : '' }}>{{$rig->Companies->name ." " . $rig->name }}</option>
                                                            @endforeach
                                                        </select>
                                                        <span
        ...
        <div class="modal fade" id="showModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header bg-light p-3">
                        <h5 class="modal-title"
                            id="exampleModalLabel">{{ trans('Dashboard/orders.create_new_order') }}</h5>
                            ...
                            <div class="mb-3">
                                <div class="row">
                                    <div class="col-12">
                                        ...
                                        <input type="date" id="date" name="date" class="form-control"
                                               placeholder="{{ trans('Dashboard/orders.placeholderDate') }}" required>
                                        <span
                                            class="form-text text-muted">{{ trans('Dashboard/orders.orderDate') }} </span>
                                    </div>
                                </div>
                                <br>
                                <br>
                                <div class="row">
                                    ...
                                    <div class="col-6">
                                        <label for="rig_id"
                                               class="form-label">{{ trans('Dashboard/orders.rig') }}</label>
                                        <select class="form-control @error('rig_id') is-invalid @enderror" id="rig_id"
                                                name="rig_id">
                                            <option value="" selected disabled>Choose Company</option>
                                            @foreach ($rigs as $rig)
                                                <option
                                                    value="{{ $rig->id }}">{{ $rig->Companies->name ." " .$rig->name }}</option>
                                            @endforeach
                                        </select>
                                        <span
                                            class="form-text text-muted">{{ trans('Dashboard/orders.orderRig') }}</span>

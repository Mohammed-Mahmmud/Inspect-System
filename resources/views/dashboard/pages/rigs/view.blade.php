    ...
    <div class="page-content">
        <div class="container-fluid">
            <x-dashboard.layouts.breadcrumb title1="{{ trans('Dashboard/rigs.rigs') }}"
                                    title2="{{ trans('Dashboard/rigs.rigs') }}"
                                    route="{{route('rig.index')}}"
                            ...
                            <div id="customerList">
                                <div class="row g-4 mb-3">
                                    <div class="col-sm-auto">
                                        <div>
                                            <a class="btn btn-success add-btn" href="{{ route('rig.create') }}" data-bs-toggle="modal" data-bs-target="#showModal">{{ trans('Dashboard/rigs.addrig') }}</a>
    ...
    <table class="table align-middle mb-0">
        <thead class="table-dark">
                                            <tr>
                                                ...
                                                <th class="sort" data-sort="customer_name">{{ trans('Dashboard/rigs.name') }}</th>
                                                <th class="sort" data-sort="customer_full_name">{{ trans('Dashboard/rigs.location') }}</th>
                                                <th class="sort" data-sort="customer_full_name">{{ trans('Dashboard/rigs.company') }}</th>
                                                <th class="sort" data-sort="action">{{ trans('Dashboard/rigs.create_date') }}</th>
                                                <th class="sort" data-sort="action">{{ trans('Dashboard/rigs.action') }}</th>
                                            </tr>
                                        </thead>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="d-flex gap-2">
                                                        ...
                                                        <!-- Modal -->
                                                        <form action="{{ route('rig.destroy', $item) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                                ...
                                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                                    <div class="modal-content">
                                                                        <div class="modal-header">
                                                                            <h5 class="modal-title" id="exampleModalLongTitle">{{ trans('Dashboard/rigs.remove') }} {{ $item->name}}</h5>
                                                                        ...
                                                                        <div class="modal-footer">
                                                                            <div class="hstack gap-2 justify-content-end">
                                                                                <button type="button" class="btn btn-info" data-bs-dismiss="modal">{{ trans('Dashboard/rigs.close') }}</button>

                                                                                <button type="submit" class="btn btn-danger" id="add-btn">{{ trans('Dashboard/rigs.delete_rig') }}</button>
                                                        </form>
                                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header bg-light p-3">
                                    <h5 class="modal-title" id="exampleModalLabel2">{{ trans('Dashboard/rigs.edit').' '.$item->name }}</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" id="close-modal"></button>
                                </div>
                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            <div class="row">
                                                <div class="row">
                                                    <div class="col-6">
                                                        <label for="customername-field" class="form-label">{{ trans('Dashboard/rigs.name') }}</label>
                                                        <input type="text" id="customername-field" name="name" class="form-control" placeholder="{{ trans('Dashboard/rigs.placeholderName') }}" value="{{ $item->name }}" required="">
                                                    </div>
                                                    <div class="col-6">
                                                        <label for="customername-field" class="form-label">{{ trans('Dashboard/rigs.company') }}</label>
                                                        <select class="form-control @error('company_id') is-invalid @enderror" id="company_id" name="company_id">
                                                            <option value="{{ $item->companies->id }}" selected disabled>{{ $item->companies->name }}</option>
                                                            @foreach ($companies as $company)
                                                            <option value="{{ $company->id }}" {{ $company->id == $item->companies->id ? 'selected' : '' }}>{{ $company->name }}</option>
                                                            @endforeach
                                                        </select>
                                                ...
                                                <div class="row">
                                                    <div class="col-12">
                                                        <label for="customername-field" class="form-label">{{ trans('Dashboard/rigs.location') }}</label>
                                                        <textarea id="customername-field" name="location" class="form-control" placeholder="{{ trans('Dashboard/rigs.placeholderLocation') }}" required>
                                                        {{ $item->location }}
                                                        </textarea>
                                        ...
                                        <div class="modal-footer">
                                            <div class="hstack gap-2 justify-content-end">
                                                <button type="button" class="btn btn-danger" data-bs-dismiss="modal">{{ trans('Dashboard/rigs.close') }}</button>

                                                <button type="submit" class="btn btn-info" id="add-btn">{{ trans('Dashboard/rigs.update_rig') }}</button>
                                </form>
                ...
                <form class="tablelist-form" action="" method="">
                    <div class="modal-body">
                        <div class="mb-3">
                            <div class="row">
                                <div class="col-6">
                                    <label for="customername-field" class="form-label">{{ trans('Dashboard/rigs.name') }}</label>
                                    <input type="text" id="customername-field" name="name" class="form-control" placeholder="{{ trans('Dashboard/rigs.placeholderName') }}" required>
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <label for="customername-field" class="form-label">{{ trans('Dashboard/rigs.location') }}</label>
                                    <textarea id="customername-field" name="location" class="form-control" placeholder="{{ trans('Dashboard/rigs.placeholderLocation') }}" required></textarea>
                    ...
                    <div class="modal-footer">
                        <div class="hstack gap-2 justify-content-end">
                            ...
                            <button type="submit" class="btn btn-success" id="add-btn">{{ trans("Dashboard/rigs.create_rig") }}</button>
                </form>
            </div>
        </div>
</form>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="d-flex gap-2 justify-content-center mt-4 mb-2">
                    <button type="button" class="btn w-sm btn-info" data-bs-dismiss="modal">{{ trans('Dashboard/rigs.close') }}</button>

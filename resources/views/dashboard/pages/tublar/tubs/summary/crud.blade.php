...
@section('css')
    <link rel="stylesheet" href="{{ asset('dashboard/pages/tublar/tubs/summaryCss/style.css') }}">
@endsection
    ...
    <div class="main-content">
        <div class="page-content">
            <div class="container-fluid">
                <x-dashboard.layouts.breadcrumb title1="{{ TranslationHelper::translate($type . '_' . ucwords($examType)) }}"
                    ...
                    <div id="customerList">
                        ...
                        <form class="tablelist-form" action="{{ $action }}" method="POST"
                            ...
                            <div class="modal-body">
                                {{-- conent --}}
                                <x-dashboard.reports.tubs.summary.content :type="$type" :examType="$examType"
                                    :data="$data" :accept="$accept"
                                    :orders="$orders"></x-dashboard.reports.tubs.summary.content>
                                {{-- end content --}}
                            </div>
                            <br>
                            <div class="modal-footer row">
                                @if ($type == 'Edit')
                                    <div class="col-8">
                                        <a type="button" style="color: black; font-weight:bold;"
                                            ...
                                            href="{{ route('tubs.reports.create', ['type' => $data->type]) }}">{{ TranslationHelper::translate(ucwords('new report')) }}</a>
                                        <a type="button" style="color: black; font-weight:bold;"
                                            class="btn btn-outline-primary"
                                            href="{{ route('tubs.reports.repeat', $data->id) }}">{{ TranslationHelper::translate(ucwords('repeat report')) }}</a>
                                    </div>
                                @endif
                                <div class="col-4 hstack gap-2 justify-content-end">
                                    ...
                                    <button style="color: black; font-weight:bold;" type="submit" class="btn btn-success"
                                        id="add-btn">{{ TranslationHelper::translate(ucwords('save')) }}</button>
                                </div>
                            </div><br>
                        </form>
                    </div>

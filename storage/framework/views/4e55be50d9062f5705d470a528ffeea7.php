    ...
    <li class="nav-item">
        <?php if(!empty($item->route)): ?>
            <a href="<?php echo e(!empty($item->type) ? route($item->route, ['type' => $item->type]) : route($item->route)); ?>"
                class="nav-link
   <?php if(Request::fullUrl() === (!empty($item->type) ? route($item->route, ['type' => $item->type]) : route($item->route))): ?> activeSidebar <?php endif; ?>"
                data-key="t-detached"><?php echo e(ucwords($item->name)); ?></a>
        <?php else: ?>
...
<?php /**PATH /home/mohamed-khater/Documents/projects/stc/inspection-system/resources/views/components/dashboard/layouts/sidebar-items.blade.php ENDPATH**/ ?>